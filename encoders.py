...
from torch.nn.modules.activation import ReLU, ELU, LeakyReLU, SELU
...
class GConvModule(nn.Module):
    ...
    pred_hidden_dims=[], concat=True, bn=True, dropout=0.0, normalize=False, num_aggs=1, att=False, 
    args=None):
        ...
        if label_dim:
            ...
            pred_model = nn.Linear(pred_input_dim, pred_input_dim)
    ...
    def forward(self, x, adj, embedding_mask=None):
        ...
        if self.att:
            return nn.Softmax(dim=-1)(ypred)
        else:
            return nn.Softmax(dim=-1)(ypred*(nn.Softmax(dim=-1)(ypred)))
...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
            assign_hidden_dim, assign_ratio=0.25, assign_num_layers=-1, num_pooling=1, num_unpooling=1,
            unpool_ratio=0.25, pred_hidden_dims=[50], concat=True, bn=True, dropout=0.0, linkpred=True,
        ...
        try:
            assert (num_unpooling==num_pooling) and (assign_ratio==unpool_ratio)
        except AssertionError:
            #num_unpooling = num_pooling
            #unpool_ratio = assign_ratio
            print("\nWarning: This is not a U-Net\n")
            print('There are {} pooling layers while {} unpooling layers'.format(num_pooling, num_unpooling))
            print('Pooling ratio is {} while unpooling ratio is {}'.format(assign_ratio, unpool_ratio))
            print("\nAbandoned UNPOOLING\n")
            num_unpooling = 0
            unpool_ratio = 1
        ...
        self.num_unpooling = num_unpooling
        self.unpool_ratio = unpool_ratio
        self.linkpred = linkpred
        self.assign_ent = True
        self.adj_sim = 0
        self.adj_lambda = 0.1#args.rcs_lambda
        ...
        self.test = GConvModule(assign_input_dim, assign_hidden_dim, assign_dim, assign_dim, num_layers=5)
        ...
        # create and initizalize unpooling layers
        self.unpool_modules = nn.ModuleList()
        self.adj_modules = nn.ModuleList()
        self.S_modules = nn.ModuleList()
        assign_dim = int(assign_dims[-1] / unpool_ratio)

        for i in range(1, num_unpooling+1):
            self.unpool_modules.append(GConvModule(input_dim=int((i+1)*assign_input_dim), 
            hidden_dim=hidden_dim, 
            embedding_dim=embedding_dim, 
            label_dim=assign_dims[-i-1], 
            num_layers=5))

            self.adj_modules.append(nn.Sequential(nn.Linear(assign_dims[-i-1], assign_dims[-i-1]),
            nn.SELU(),
            nn.Linear(assign_dims[-i-1], assign_dims[-i-1])))

            self.S_modules.append(nn.Sequential(nn.Linear(assign_dims[-i-1], assign_dims[-i-1]),
            nn.SELU(),
            nn.Linear(assign_dims[-i-1], assign_dims[-i-1],
            nn.Softmax())))

            assign_dim = int(assign_dim / unpool_ratio)
        
        # gconv after unpooling
        self.conv_after_unpool = nn.ModuleList()
        i = 0
        for i in range(num_unpooling):
            self.conv_after_unpool.append(GConvModule(input_dim=int((i+1)*assign_input_dim), 
            hidden_dim=hidden_dim, 
            embedding_dim=embedding_dim, 
            label_dim=int((i+1)*assign_input_dim), 
            num_layers=5,
            att=True))

        if num_unpooling:
            self.pred_model = self.build_pred_layers(int((i+2)*assign_input_dim), pred_hidden_dims, 
                label_dim, num_aggs=self.num_aggs)
        
        self.fcn1 = GConvModule(int(assign_input_dim*(i+2)), hidden_dim, embedding_dim, 
        int(assign_input_dim*(i+2)), 
        num_layers=5)

        self.fcn2 = GConvModule(int(assign_input_dim*(i+2)), hidden_dim, embedding_dim, 
        int(assign_input_dim*(i+2)), 
        num_layers=5)
    ...
    def forward(self, x, adj, batch_num_nodes, dupl_adj, trans_S, **kwargs):
        ...
        for i in range(self.num_unpooling):
            ...
            if trans_S:
                a = torch.transpose(pooling_S[-i-1], 1, 2)
                self.assign_tensor = self.S_modules[i](torch.transpose(pooling_S[-i-1], 1, 2))
            else:
                self.assign_tensor = self.unpool_modules[i](x_a, adj, embedding_mask)

            if embedding_mask is not None:
                self.assign_tensor = self.assign_tensor * embedding_mask

            # update pooled features and adj matrix
            embedding_tensor = self.conv_after_unpool[i](x_a, adj, embedding_mask)

            x = torch.matmul(torch.transpose(self.assign_tensor, 1, 2), embedding_tensor)

            if dupl_adj:
                adj = pooling_adj[-i-2]
                adj = Normalize(0, 1)(adj)
                adj = self.adj_modules[i](adj)
            else:    
                adj = torch.transpose(self.assign_tensor, 1, 2) @ adj @ self.assign_tensor
                adj = Normalize(0, 1)(adj)
                adj = self.adj_modules[i](adj)

            x_a = torch.cat((x, pool_outs[-i-2]), dim=-1)

        if self.num_unpooling:
            if self.adj_lambda:
                adj_g = pooling_adj[0] 
                adj_g = Normalize(0, 1)(adj_g)
                re_loss = torch.abs(adj_g - adj)
                re_loss = re_loss.view(-1, 1)
                re_loss = torch.mean(re_loss)
                self.adj_sim = re_loss
            x1 = self.fcn1(x_a, adj)
            x2 = self.fcn2(x1, adj)
            x = x_a + x2
            x = self.act(x)
    ...
    def loss(self, pred, label, adj=None, batch_num_nodes=None, adj_hop=1):
        ...
        if self.linkpred:
            ...
            return (1-adj_lambda) *(loss + self.link_loss) + adj_lambda * self.adj_sim
        
        #reconstruction_loss = -math.log(np.abs(self.adj_sim)+eps)
        reconstruction_loss = self.adj_sim
        #print('Reconstruction Loss: ', reconstruction_loss.item()) 
        return (1-adj_lambda) * loss + adj_lambda * reconstruction_loss
