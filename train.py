...
def evaluate(dataset, model, args, name='Validation', max_num_examples=None):
    ...
    for batch_idx, data in enumerate(dataset):
        ...
        dupl_adj=args.dupli_adj, trans_S=args.trans_S, args=args)
...
def train(dataset, model, args, same_feat=True, val_dataset=None, test_dataset=None, writer=None,
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, data in enumerate(dataset):
            ...
            avg_loss += loss
            #if iter % 20 == 0:
            #    print('Iter: ', iter, ', loss: ', loss.data[0].item())
            elapsed = time.time() - begin_time
            total_time += elapsed

            # log once per XX epochs
            '''
            lmx: I revised 1 to -1 here, to avoid plotting
            '''
            if epoch % 10 == -1 and batch_idx == len(dataset) // 2 and args.method == 'soft-assign' and writer is not None:
...
def syn_community1v2(args, writer=None, export_graphs=False):
    ...
    if args.method == 'soft-assign':
        ...
        model = encoders.SoftPoolingGcnEncoder(
                ...
                num_unpooling=args.num_unpool, unpool_ratio=args.ratio_unpool).cuda()
...
def syn_community2hier(args, writer=None):
    ...
    if args.method == 'soft-assign':
        ...
        model = encoders.SoftPoolingGcnEncoder(
                ...
                input_dim, args.hidden_dim, args.output_dim, args.num_classes, args.num_gc_layers,
                args.hidden_dim, assign_ratio=args.assign_ratio, num_pooling=args.num_pool,
                num_unpooling=args.num_unpool, unpool_ratio=args.ratio_unpool,
...
def benchmark_task_val(args, writer=None, feat='node-label'):
    all_vals = []
    graphs = load_data.read_graphfile(args.datadir, args.bmname, max_nodes=args.max_nodes) # have value
    ...
    for i in range(min([10, max([int(len(graphs)*(1-args.train_ratio-args.test_ratio)),1])])): 
...
def arg_parse():
    ...
    parser.add_argument('--num-unpool', dest='num_unpool', type=int,
            help='number of unpooling layers')
    parser.add_argument('--unpool-ratio', dest='ratio_unpool', type=float,
            help='unpooling ratio')      

    parser.add_argument('--weight-decay', dest='weight_decay',  type=float,
            help='lambda in L2 regularization')        

    parser.add_argument('--duplicate-adj', dest='dupli_adj',  type=bool,
            help='whether to use pooling adjacent matrix in unpooling')
    parser.add_argument('--trans-s', dest='trans_S',  type=bool,
            help='whether transform S') 

    parser.add_argument('--rcs-l', dest='rcs_lambda', type=float,
    help='the ratio of reconstruction error')            
