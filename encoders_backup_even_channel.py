...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        for i in range(1, num_unpooling+1):
            self.unpool_modules.append(GConvModule(input_dim=assign_input_dim, 
        ...
        for i in range(num_unpooling):
            self.conv_after_unpool.append(GConvModule(input_dim=2*assign_input_dim, 
            hidden_dim=hidden_dim, 
            embedding_dim=embedding_dim, 
            label_dim=assign_input_dim, 
            num_layers=5,
            att=True))

        if num_unpooling:
            self.pred_model = self.build_pred_layers(assign_input_dim, pred_hidden_dims, 
                label_dim, num_aggs=self.num_aggs)
        
        self.fcn1 = GConvModule(assign_input_dim, hidden_dim, embedding_dim, 
        assign_input_dim, 
        num_layers=5)

        self.fcn2 = GConvModule(assign_input_dim, hidden_dim, embedding_dim, 
        assign_input_dim, 
        num_layers=5)
    ...
    def forward(self, x, adj, batch_num_nodes, dupl_adj, trans_S, **kwargs):
        ...
        for i in range(self.num_unpooling):
            ...
            x_a = self.conv_after_unpool[i](x_a, adj, embedding_mask)
    ...
    def loss(self, pred, label, adj=None, batch_num_nodes=None, adj_hop=1):
        ...
        if self.linkpred:
            ...
            return loss + self.link_loss + adj_lambda * self.adj_sim
